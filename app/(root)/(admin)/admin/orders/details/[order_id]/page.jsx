...
const OrderDetails = async ({ params }) => {
    ...
    const { data: orderDetailsData } = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/orders/get/${order_id}`)
    return (
        <div>
                ...
                {orderDetailsData && !orderDetailsData?.success
                    ...
                    <div>
                        <div className='mb-5'>
                            ...
                            <p className='capitalize'><b>Status: </b>{orderDetailsData?.data?.status}</p>
                        </div>
                        <table className='w-full border'>
                            ...
                            <tbody>
                                {orderDetailsData && orderDetailsData?.data?.products?.map((product) => (
                                    ...
                                    <table className='w-full'>
                                        <tbody>
                                            <tr>
                                                ...
                                                <td className='text-end py-2'>{orderDetailsData?.data?.subTotal.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                            </tr>
                                            <tr>
                                                <td className='font-medium py-2'>Discount</td>
                                                <td className='text-end py-2'>{orderDetailsData?.data?.discount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                            </tr>
                                            <tr>
                                                <td className='font-medium py-2'>Coupon Discount</td>
                                                <td className='text-end py-2'>{orderDetailsData?.data?.couponDiscountAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                            </tr>
                                            <tr>
                                                <td className='font-medium py-2'>Total</td>
                                                <td className='text-end py-2'>{orderDetailsData?.data?.totalAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
